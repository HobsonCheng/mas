...
//    }
    ...
    @objc func dy_present(_ viewControllerToPresent: UIViewController?, animated flag: Bool, completion: @escaping () -> Void) {
        if (viewControllerToPresent is UIAlertController) {
            print("title : \((viewControllerToPresent as? UIAlertController)?.title ?? "")")
            print("message : \((viewControllerToPresent as? UIAlertController)?.message ?? "")")
