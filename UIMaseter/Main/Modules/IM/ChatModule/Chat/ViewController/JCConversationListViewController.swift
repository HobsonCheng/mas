...
class JCConversationListViewController: NaviBarVC {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        super.viewDidLoad()
        self.naviBar()?.setTitle(title: "会话列表")
    ...
    fileprivate lazy var tableview: UITableView = {
        var tableview = UITableView(frame: CGRect(x: 0, y: self.naviBar()?.bottom ?? 64, width: self.view.width, height: self.view.height - (self.naviBar()?.bottom ?? 64)))
    ...
    fileprivate lazy var emptyView: UIView = {
        let view = UIView(frame: CGRect(x: 0, y: self.naviBar()?.bottom ?? 64, width: self.view.width, height: self.view.height - (self.naviBar()?.bottom ?? 64)))
        view.isHidden = true
...
extension JCConversationListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if self.systemDatas.count > 0  && indexPath.section == 0{
            ...
            MBMasterHUD.showLoading(title: "申请加入中...")
            ...
            ApiUtil.share.applyGroup(params: params, finish: { (status, data, msg) in
                ...
                if B_ResponseStatus.success == status {
                    ...
                    MBMasterHUD.showFail(title: msg ?? "失败")
                }
            })
...
extension JCConversationListViewController {
    ...
    fileprivate func request_groups(){
        ...
        MBMasterHUD.showLoading(title: "获取系统群列表...")
