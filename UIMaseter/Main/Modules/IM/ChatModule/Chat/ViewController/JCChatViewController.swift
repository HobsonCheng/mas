...
class JCChatViewController: NaviBarVC {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if let group = conversation.target as? JMSGGroup {
            self.naviBar()?.setTitle(title: group.displayName())
        }
    ...
    private func _updateTitle() {
        if let group = conversation.target as? JMSGGroup {
            ...
            self.naviBar()?.setTitle(title: conversation.title ?? "")
        }
    }
    ...
    private func _setupNavigation() {
        let navButton: NaviBarItem
        if isGroup {
            navButton = NaviBarItem.init(imageSize: CGSize.init(width: 44, height: 20), target: self, action:  #selector(_getGroupInfo))
            navButton.setIconImage(with: UIImage.loadImage("com_icon_group_w")!, for: .normal)
        } else {
            navButton = NaviBarItem.init(imageSize: CGSize.init(width: 44, height: 20), target: self, action:  #selector(_getSingleInfo))
            navButton.setIconImage(with: UIImage.loadImage("com_icon_user_w")!, for: .normal)
        }
       
        self.naviBar()?.setRightBarItems(with: navButton)
        
        navigationController?.interactivePopGestureRecognizer?.isEnabled = true
        navigationController?.interactivePopGestureRecognizer?.delegate = self
    }
    
    @objc override func goBack(_ sender: AnyObject) {
        if isTop {
    ...
    func onSendMessageResponse(_ message: JMSGMessage!, error: Error!) {
        if let error = error as NSError? {
            if error.code == 803009 {
                MBMasterHUD.showFail(title: "发送失败，消息中包含敏感词")
            }
            if error.code == 803005 {
                MBMasterHUD.showFail(title: "您已不是群成员")
            }
        }
...
extension JCChatViewController: YHPhotoPickerViewControllerDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
    func selectedPhotoBeyondLimit(_ count: Int32, currentView view: UIView!) {
        MBMasterHUD.showInfo(title:  "最多选择\(count)张图片")
    }
    ...
    func withdrawMessage(message: JCMessageType) {
        ...
        JMSGMessage.retractMessage(message, completionHandler: { (result, error) in
            if error == nil {
                ...
                MBMasterHUD.showInfo(title: "发送时间过长，不能撤回")
            }
        })
    }
