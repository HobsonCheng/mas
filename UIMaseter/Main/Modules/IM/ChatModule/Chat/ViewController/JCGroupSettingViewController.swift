...
class JCGroupSettingViewController: NaviBarVC {
    ...
    //MARK: - private func
    private func _init() {
        self.naviBar()?.setTitle(title: "群组信息")
...
//        && group.ownerAppKey == user.appKey!  这里group.ownerAppKey == "" 目测sdk bug
        ...
        tableView.frame = CGRect(x: 0, y: self.naviBar()?.bottom ?? 64, width: view.width, height: view.height - (self.naviBar()?.bottom ?? 64))
...
extension JCGroupSettingViewController: UIAlertViewDelegate {
    func alertView(_ alertView: UIAlertView, clickedButtonAt buttonIndex: Int) {
        switch buttonIndex {
        case 1:
            MBMasterHUD.showLoading(title: "退出中...")
...
//        }
        ...
        if actionSheet.tag == 1001 {
            if buttonIndex == 1 {
                ...
                MBMasterHUD.showSuccess(title: "成功清空")
            }
        }
    }
}
