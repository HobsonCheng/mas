...
class LightControl: UIControl {
    ...
    override func draw(_ rect: CGRect) {
        ...
        if isEnabled == false{
            imageDisable?.draw(in: CGRect.init(x: 0, y: 0, width: width, height: height))
        }else if isHighlighted == true{
            if let imageHi = imageHighlight{
                imageHi.draw(in: CGRect.init(x: 0, y: 0, width: width, height: height))
            }
        }else{
            imageNormal?.draw(in: CGRect.init(x: 0, y: 0, width: width, height: height))
        }
    }
