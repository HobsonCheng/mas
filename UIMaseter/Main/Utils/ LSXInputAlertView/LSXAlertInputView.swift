...
class LSXAlertInputView: UIView, UITextViewDelegate {
    ...
    fileprivate var type = LSXKeyboardType.default
    fileprivate var alertView: UIView?
    fileprivate var titLa: UILabel?
    fileprivate var textView: LSXPlacehoderTextView?
    ...
    fileprivate lazy var shadowView: UIView? = {
        ...
        shadowView.backgroundColor = RGBA_COLOR(0, 0, 0, 0.4)
    ...
    ///   - bordtype: 键盘类型
    ///   - completeBlock: 点击回调
    convenience init(title: String?, placeholderText PlaceholderText: String?, withKeybordType bordtype: LSXKeyboardType, completeBlock: @escaping (_ contents: String?) -> Void) {
        self.init()
        self.title = title
...
//        }
    ...
    func creatUI() {
        ...
        qxbtn = UIButton()
        guard let qxbtn = self.qxbtn else { return }
