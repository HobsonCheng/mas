...
class SearchBarFiled: UITextField {
    ...
    override init(frame: CGRect) {
        ...
        self.placeholder = "请输入您单位的名称、字号或APP编号"
    ...
    fileprivate func setLeftViewWithImage() {
        ...
        imgView.setYJIconWithName(icon: .search, textColor: .white,size:CGSize.init(width: 18, height: 18))
    ...
    func setClearButtonImage(){
        let btn = self.value(forKey: "clearButton") as! UIButton
        btn.setImage(UIImage.init(icon: .clear, size: CGSize.init(width: 18, height: 18), orientation: .up, textColor: .white, backgroundColor: .clear), for: .normal)
    }
